...
def compute_basic_metrics(sales: List[Sale]) -> Dict[str, float]:
    ...
    trend_candidates = [(trend_rel_30_points, slope_points)]
    # Для прогноза на нисходящем тренде допускаем недавний тренд только если он
    # не меняет знак относительно 30-дневного, чтобы не переворачивать прогноз.
    if trend_rel_30_recent == 0.0 or math.copysign(1.0, trend_rel_30_recent) == math.copysign(
        1.0, trend_rel_30_points
    ):
        trend_candidates.append((trend_rel_30_recent, slope_points_recent))
    trend_rel_30_down_forecast, slope_points_down_forecast = min(
        trend_candidates, key=lambda t: abs(t[0])
    )
    ...
    return {
        ...
        "trend_rel_30_down_forecast": trend_rel_30_down_forecast,
        "slope_per_day": slope_points,
        "slope_per_day_unweighted": slope_points,
        "slope_per_day_volume": slope_volume,
        "slope_per_day_down_forecast": slope_points_down_forecast,
