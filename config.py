...
# Подсказка по текущему балансу аккаунта Lisskins. Используется только для
# приблизительного расчёта минимальной цены после достижения 800 активных лотов.
# Если баланс не известен, оставьте значение None.
LISSKINS_BALANCE_USD = None

# Таймаут для приоритезации свежих лотов из websocket (секунды).
WEBSOCKET_PRIORITY_WINDOW = 10.0

# Размер очередей и количество воркеров.
ANALYSIS_WORKERS = 4
PURCHASE_WORKERS = 2
